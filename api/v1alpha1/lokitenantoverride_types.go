...
// LokiTenantOverrideSpec defines a per-tenant override contribution for Loki.
// The operator deep-merges Spec.Overrides into the tenant's stanza of the
// output ConfigMap.
type LokiTenantOverrideSpec struct {
	...
	// validation against Loki's upstream Limits happens in the admission
...
// LokiTenantOverrideStatus reflects the controller's view of this CR.
type LokiTenantOverrideStatus struct {
	...
	// ContributingPeers lists other LokiTenantOverride CRs that target
	// the same tenantId.
	// +optional
...
// +kubebuilder:resource:scope=Namespaced,shortName=lto
...
// LokiTenantOverride is a per-tenant runtime override contribution for Loki.
type LokiTenantOverride struct {
	...
	// spec defines the desired state of LokiTenantOverride
	// +required
	Spec LokiTenantOverrideSpec `json:"spec"`

	// status defines the observed state of LokiTenantOverride
	// +optional
	Status LokiTenantOverrideStatus `json:"status,omitzero"`
}

// +kubebuilder:object:root=true

// LokiTenantOverrideList contains a list of LokiTenantOverride
type LokiTenantOverrideList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitzero"`
	Items           []LokiTenantOverride `json:"items"`
}

func init() {
	SchemeBuilder.Register(&LokiTenantOverride{}, &LokiTenantOverrideList{})
}
