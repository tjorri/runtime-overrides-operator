...
var _ = BeforeSuite(func() {
	...
	// mgr.Start(); the webhook tests below run in-process and depend on
	// the same package-global state.
	validate.InitDefaults()

	ctx, cancel = context.WithCancel(context.TODO())

	var err error
	err = runtimeoverridesv1alpha1.AddToScheme(scheme.Scheme)
	...
	err = SetupLokiTenantOverrideWebhookWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())

	err = SetupMimirTenantOverrideWebhookWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())
